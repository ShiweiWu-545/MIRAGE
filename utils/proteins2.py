...
from utils.residue_constants import restypes_with_x, restype_order_with_x, STANDARD_ATOM_MASK, restype_3to1,\
    restype_order, restype_num, atom_type_num, atom_types, atom_order
from utils.atom_convert import atom37_to_torsion7_np, atom37_to_atom14
...
class ProteinInput:
    ...
    atom_positions: Optional[np.ndarray] = field(default=None)
    residue_index: Optional[np.ndarray] = field(default=None)
    b_factors: Optional[np.ndarray] = field(default=None)

    torsion_angles_sin_cos: Optional[np.ndarray] = field(default=None)
    alt_torsion_angles_sin_cos: Optional[np.ndarray] = field(default=None)
    torsion_angles_mask: Optional[np.ndarray] = field(default=None)

    chainid: Optional[np.ndarray] = field(default=None)

    chainseq: Optional[np.ndarray] = field(default=None)
    ...
    def padding(self, pad_width):
        if pad_width > 0:
            ...
            array_padding_settings = {
                'atom_positions': dict(pad_width=((0, pad_width), (0, 0), (0, 0)), mode='constant', constant_values=1000),
                ...
                'alt_torsion_angles_sin_cos': dict(pad_width=((0, pad_width), (0, 0)), mode='edge'),
                'torsion_angles_mask': dict(pad_width=((0, pad_width), (0, 0), (0, 0)), mode='edge'),
    ...
    def from_pdb(cls, path, with_angles=True, return_dict=False, verbose=False):
        ...
        for chain_seq, chain in enumerate(chains):
            try:
                chainid = chain.id
...
def proteins_merge(proteins, chainids=None):
    ...
    if chainids is None:
        ...
        chain_arr = np.concatenate([[i] * p.length for i, p in enumerate(proteins)])
    else:

        chain_arr = np.concatenate([[i] * p.length for i, p in zip(chainids, proteins)])
        chainseq = [i + 1 for i in range(len(chainids))]
        chain_seq = np.concatenate([[i] * p.length for i, p in zip(chainseq, proteins)])
    p = dataclasses.replace(p, chainid=chain_arr)
    p = dataclasses.replace(p, chainseq=chain_seq)
    return p
