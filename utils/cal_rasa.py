...
def parse_chain_ids(chains):
    if isinstance(chains, str):
        if ',' in chains:
            return [chain.strip() for chain in chains.split(',') if chain.strip()]
        return list(chains)
    return list(chains)
...
def save_selected_chains(pdb_path, chain_ids, out_path):
    parser = PDBParser(QUIET=1)
    structure = parser.get_structure(None, str(pdb_path))
    io = PDBIO()
    io.set_structure(structure)
    io.save(str(out_path), ChainSelect(chain_ids))
    if out_path.stat().st_size == 0:
        raise ValueError(f"No matching chains {chain_ids} found in {pdb_path}.")
...
def analyse_asa(asa_c,asa_0 = None,asa_1=None):
    ...
    df_1['protein'] = 1
    df_m = pd.concat([df_0,df_1], ignore_index=True)
    ...
    tem_file = tem_file.rsplit('_', 1)[0]
...
def get_region_2(name,proteinA,proteinB,out_dir):
    ...
    save_selected_chains(pdb, parse_chain_ids(proteinA), pdb_0)
    save_selected_chains(pdb, parse_chain_ids(proteinB), pdb_1)
    asa_0 = cal_asa_bio(pdb_0,out_dir)
    asa_1 = cal_asa_bio(pdb_1,out_dir)
    asa_c = cal_asa_bio(pdb,out_dir)
    analyse_asa(asa_c,asa_0,asa_1)

def get_region_ske(pdb_path,proteinA,proteinB):
...
def remove_TER_lines(pdb_path):

    with open(pdb_path, 'r') as file:
        lines = file.readlines()


    filtered_lines = [line for line in lines if not line.startswith('TER')]
