...
from utils.geometry import apply_rigid
from utils.geometry import multi_rigid
from utils.geometry import get_dih
...
def atom37_to_torsion7_np(aatype, all_atom_pos, all_atom_mask):
    ...
    pre_omega_atom_pos = np.concatenate([all_atom_pos_prev[..., [1, 2], :], all_atom_pos[..., [0, 1], :]],
                                        axis=-2)
    phi_atom_pos = np.concatenate([all_atom_pos_prev[..., [2], :], all_atom_pos[..., [0, 1, 2], :]], axis=-2)
    ...
    angles = get_dih(pos[:, :, 0], pos[:, :, 1], pos[:, :, 2], pos[:, :, 3])
    angles = angles.numpy()
    ...
    pre_omega_atom_mask = np.concatenate([all_atom_mask_prev[..., [1, 2]], all_atom_mask[..., [0, 1]]],
                                         axis=-1)
    phi_atom_mask = np.concatenate([all_atom_mask_prev[..., [2]], all_atom_mask[..., [0, 1, 2]]], axis=-1)
...
def torsion7_to_frame8_np(aatype, sin_cos):
    ...
    all_rots = all_rots.reshape(-1, 8, 3, 3)
...
class TorsionPositionTransformer(nn.Module):
    ...
    def atom37_to_torsion7_torch(self, aatype, all_atom_pos, all_atom_mask):
        ...
        all_atom_mask_prev[:, 0] = 0
        atom_indices = self.chi_atom_indices_pt[aatype]
        ...
        phi_atom_pos = torch.cat([all_atom_pos_prev[..., [2], :], all_atom_pos[..., [0, 1, 2], :]],
                                 dim=-2)
        psi_atom_pos = torch.cat([all_atom_pos[..., [0, 1, 2, 4], :]], dim=-2)
        ...
        pre_omega_atom_mask = torch.cat([all_atom_mask_prev[..., [1, 2]], all_atom_mask[..., [0, 1]]],
                                        dim=-1)
        phi_atom_mask = torch.cat([all_atom_mask_prev[..., [2]], all_atom_mask[..., [0, 1, 2]]], dim=-1)
        psi_atom_mask = torch.cat([all_atom_mask[..., [0, 1, 2, 4]]], dim=-1)
    ...
    def torsion7_to_frame8_torch(self, aatype: torch.Tensor, sin_cos: torch.Tensor):
        default_frames = self.restype_rigid_group_default_frame_torch[aatype]
        default_rot = default_frames[..., :3, :3]
        default_trans = default_frames[..., :3, 3]
    ...
    def local_frame8_to_local_atom37_torch(self, aatype: torch.Tensor, r: torch.Tensor, t: torch.Tensor):
        residx_to_group_idx = self.restype_atom37_to_rigid_group_torch[aatype]
        lit_positions = self.restype_atom37_rigid_group_positions_torch[aatype]
    ...
    def atom37_to_atom14_torch(self, aatype: torch.Tensor, all_atom_pos: torch.Tensor):
        ...
        atom_indices = restype_atom37_to_atom14_index_torch[aatype]
